...
export const useStudioPage = (slug: string | undefined): UseStudioPageReturn => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        const response = await fetch(`${baseURL}/studio/${slug}`);
        ...
        const { studio: studioRaw, services: servicesRaw, results: resultsRaw, reviews: reviewsRaw } = json.data;
        ...
        // Format services
        const formattedServices: ServiceData[] = (servicesRaw || []).map((item: any) => ({
          id: String(item.id),
          name: item.name,
        ...
        const parsedReviews: ReviewData[] = (reviewsRaw || []).map((r: any) => ({
          ...r,
        ...
        setError(err instanceof Error ? err.message : "Erro ao buscar dados do studio");
      } finally {
        setLoading(false);
      }
    };
