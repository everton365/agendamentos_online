...
const TestimonialsSection = ({ reviews }: TestimonialsSectionProps) => {
  ...
  const averageRating = (reviews.reduce((sum, r) => sum + r.estrelas, 0) / reviews.length).toFixed(1);

  return (
    <section className="py-24 bg-background">
      <div className="container mx-auto px-6">
        <div className="text-center mb-20 animate-fade-in">
          <h2 className="text-4xl font-cinzel font-bold text-foreground mb-6" data-aos="fade-top">
            O que nossas{" "}
          ...
          <p className="text-xl font-poppins text-muted-foreground max-w-3xl mx-auto" data-aos="fade-top">
            A satisfação das nossas clientes é nossa maior conquista. Confira
            alguns depoimentos de quem já passou por aqui.
          </p>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10" data-aos="fade-top">
              ...
              <CardContent className="p-10">
                <div className="flex items-center mb-4">
                  {[...Array(review.estrelas)].map((_, i) => (
                    <Star key={i} className="w-5 h-5 text-elegant-gold fill-current" />
                  ))}
                </div>
        ...
        {!showAll && reviews.length > 5 && (
          <div className="text-center mt-10">
            <Button variant="outline" onClick={() => setShowAll(true)} className="font-poppins">
        ...
        <div className="text-center mt-20">
          <div className="inline-flex items-center gap-3 bg-gradient-to-r from-black to-yellow-400 text-white px-10 py-5 rounded-2xl shadow-elegant">
            <Star style={{ color: "#D4AF37" }} className="w-7 h-7 fill-current" />
            <span className="text-xl font-poppins font-semibold">{averageRating}/5 estrelas</span>
            <span className="text-white/80 font-poppins">• {reviews.length} avaliações</span>
          </div>
        </div>
      </div>
    </section>
